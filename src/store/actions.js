...
export const initIngredients = (ingredients) => {
  return (dispatch) => {
    return axios
      .get("https://burger-builder-25101999.firebaseio.com/ingredients.json")
      .then((response) => {
...
export const purchaseBurger = (orderData, idToken) => {
  return (dispatch) => {
    ...
    axios
      .post(
        `https://burger-builder-25101999.firebaseio.com/orders.json?auth=${idToken}`,
        orderData
      )
      .then((response) => {
...
export const fetchOrderStart = (idToken) => {
  return (dispatch) => {
    axios
      .get(
        "https://burger-builder-25101999.firebaseio.com/orders.json?auth=" +
          idToken
      )
      .then((response) => {
...
export const authStart = (data, type) => {
  return (dispatch) => {
    if (type === "signup") {
      axios
        .post(
          "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyAIhHeh7EChNtyK3BW2JDH1iAoI9uUhbKA",
          data
        )
      ...
      axios
        .post(
          "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAIhHeh7EChNtyK3BW2JDH1iAoI9uUhbKA",
          data
        )
        .then(function (response) {
          dispatch(authSuccess(response.data));
